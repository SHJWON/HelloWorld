...
<body>
	...
	<div id="wrap">
		...
		<div id="wrap">
			...
			<div id="search">
				<a href="<c:url value='/board/boardSearchForm/'/>">
					<button type="submit" id="boardsubmit">
						<i class="fa fa-search"></i>
					</button>
				</a>
			</div>
			<c:if test="${brdTitle eq ''}">
				<a href="<c:url value='/board/boardInsert/${ctgNo}'/>"><button
						id="insertBtn">글작성</button></a>
			</c:if>
			<c:if test="${brdTitle ne ''}">
				<a href="<c:url value='/board/boardInsert/${ctgNo}'/>"><button
						id="hiddenBtn">글작성</button></a>
			</c:if>
				...
				<tbody>
					<c:forEach var="brd" items="${brdList }">
						...
						<tr>
							<td>${brd.brdNo }</td>
							<td class="title1"><a
								href="<c:url value='/board/boardDetailView/${brd.brdNo}'/>">${brd.brdTitle }</a></td>
							<td>${brd.brdWriter }</td>
							<td><fmt:formatDate value="${brd.brdDate }"
			...
			<div class="page">
				<c:forEach begin="1" end="${pageNum}" var="num">
					<span> <a href="/board/boardListAll/${num}"><button
								id="pageBtn">${num }</button></a>
					</span>
				</c:forEach>
			</div>
		</div>
